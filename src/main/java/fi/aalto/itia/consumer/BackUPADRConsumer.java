...
public class BackUPADRConsumer extends SimulationElement {
    /**
	 * 
	 */
    private static final Logger logger = LoggerFactory.getLogger(BackUPADRConsumer.class);
    ...
    //USE policies parameter, used to force the consumers to use some policies instead of the aggregator instructions
    private boolean usePolicies = false;
    ...
    public BackUPADRConsumer(int ID, FridgeModel fridge) {
    ...
    //constructor adding the Use Policy parameter
    public BackUPADRConsumer(int ID, FridgeModel fridge, boolean usePolicies) {
	this(ID, fridge);
	this.setUsePolicies(usePolicies);
    }
